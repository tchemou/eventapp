...
class _CheckoutScreenState extends State<CheckoutScreen> {
  ...
  double get _totalPrice {
    ...
    for (var entry in _selectedQuantities.entries) {
      final ticketType =
          widget.event.ticketTypes?.firstWhere((tt) => tt.id == entry.key);
      if (ticketType != null) {
  ...
  // ✅ FIX : available est int? → remplacé par int avec valeur par défaut 0
                    ...
                    child: Row(
                      children: [
                        Expanded(
                          child: Column(
                            ...
                            children: [
                              ...
                              // ✅ FIX : description n'existe pas dans TicketType
                              // → affiche le nombre de places disponibles à la place
                              Text(
                                available > 0
                        ...
                        Row(
                          children: [
                            IconButton(
                              // ✅ FIX : passer available (int) au lieu de ticket.availableQuantity (int?)
                              onPressed: () =>
                                  _updateQuantity(ticket.id, -1, available),
                            ...
                            IconButton(
                              ...
                              // ✅ FIX : qty < available (int) au lieu de ticket.availableQuantity (int?)
